...
function [evol_obj, evol_X_aveg, Q, std_X_filter] = xFilter_Sun_19_cann_prob(GradFPair,X_xFilter_init, flag,fix_factor_1,fix_factor_2,num_iter,num_iter_last_hist, numE , numV, Lap_line_G, delta, E1,E2,target,L)
    ...
    for e = 1:numE
        grad = GradFPair( reshape(X_xFilter(:,e),dim,numV) ,e,delta,target, E1,E2);
    ...
    for t = 1:num_iter
        ...
        if (flag == 1) %this allows us to turn the filtering step on or off
            ...
            X_xFilter = ((R)\d_xfilter')'; % if we use this equation then we are basically doing step S2 in a centralized manner. This should help the algorithm
        end
